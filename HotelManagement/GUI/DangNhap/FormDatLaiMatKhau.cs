...
namespace HotelManagement.GUI
{
    public partial class FormDatLaiMatKhau : Form
    {
        ...
        private void ButtonOK_Click(object sender, EventArgs e)
        {
            ...
            {
                CTMessageBox.Show("Please enter password", "Notificatrions", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (this.textBoxPassword.Texts != this.textBoxPassConfirm.Texts)
            {
                CTMessageBox.Show("Please check password confirm", "Notificatrions", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            else
            {
                taiKhoan.Password=this.textBoxPassword.Text;
                try
                {
                    TaiKhoanBUS.Instance.AddOrUpdateTK(taiKhoan);
                    CTMessageBox.Show("Password changed successfully", "Notification", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception ex)
                {
        ...
        private void FormDatLaiMatKhau_Load(object sender, EventArgs e)
        {
