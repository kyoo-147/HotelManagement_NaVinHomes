...
// Name Server: DESKTOP-PGC8KG2\SQLEXPRESS
// Name BTb: HotelManagement
...
namespace HotelManagement.GUI
{
    public partial class FormKetNoiCSDL : Form
    {
        ...
        private void buttonKetNoi_Click(object sender, EventArgs e)
        {
            ...
            {
                CTMessageBox.Show("Cannot be left blank! Please retry.", "Notifications", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            string connectionString = string.Format("",server,db);
            try
            {
                ...
                if (sqlHelper.isConnected)
                {
                    ...
                    {
                        this.Hide(); formLogin.ShowDialog(); this.Close();
                    }
                }
            }
            catch (Exception ex)
            {
                CTMessageBox.Show("Error! An error occured. Please try again later! Please restart the program.", "Notifications", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
